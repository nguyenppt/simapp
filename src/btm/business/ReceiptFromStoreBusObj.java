...
public class ReceiptFromStoreBusObj {
  public ReceiptFromStoreBusObj() {
  }
...
// }
  ...
  public ResultSet getDataWithDate(String sDate,DataAccessLayer DAL,Statement stmt,String fromStore, String toStore){
    ...
    String sql = "";
    try{
        sql = "select RECEIPT_ID, s1.name, s2.name"+
            " from BTM_IM_RECEIPT_F_TSF t, btm_im_store s1, btm_im_store s2 where t.FROM_STORE_ID = s1.STORE_ID and t.TO_STORE_ID = s2.STORE_ID and TRANSFER_DATE = to_date('"+sDate+"','DD-MM-YYYY')";
...
//        System.out.println(sql);
  ...
  public ResultSet getDataReceiptWithID(String sID,DataAccessLayer DAL,Statement stmt){
    ResultSet rs=null;
    try{
        String sql = "Select RECEIPT_ID, s1.NAME FROM_STORE_ID, s2.NAME TO_STORE_ID, USER_ID, to_char(RECEIPT_DATE,'DD/MM/YYYY') RECEIPT_DATE"+
                     " from BTM_IM_RECEIPT_F_TSF t, BTM_IM_STORE s1, BTM_IM_STORE s2 Where t.FROM_STORE_ID = s1.STORE_ID and t.TO_STORE_ID = s2.store_id and RECEIPT_ID = '"  + sID + "'";
//        System.out.println(sql);
        rs = DAL.executeScrollQueries(sql,stmt);
      }catch(Exception e){
        ExceptionHandle eh = new ExceptionHandle();
...
//        System.out.println(sql);
  ...
  ResultSet rs=null;
  try{
      String sql = "Select im.Item_id, im.art_no, im.item_name, ad.ATTR_DTL_DESC, QTY "+
          " from BTM_IM_RECEIPT_F_TSF_ITEM t, BTM_IM_ITEM_MASTER im, BTM_IM_ATTR_DTL ad Where  t.item_id = im.item_id and im.ATTR2_DTL_ID = ad.ATTR_DTL_ID and RECEIPT_ID = '"  + sID + "' order by seq";
//      System.out.println(sql);
      rs = DAL.executeScrollQueries(sql);
    }catch(Exception e){
      ExceptionHandle eh = new ExceptionHandle();
